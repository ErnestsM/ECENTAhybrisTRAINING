...
<form:form method="post" modelAttribute="registerForm" action="${action}">
    ...
    <c:if test="${ not empty consentTemplateData }">
        <form:hidden path="ecentaConsentForm.consentTemplateId" value="${consentTemplateData.id}" />
        <form:hidden path="ecentaConsentForm.consentTemplateVersion" value="${consentTemplateData.version}" />
        <div class="checkbox">
            <label class="control-label uncased">
                <form:checkbox path="ecentaConsentForm.consentGiven" disabled="true"/>
