...
import de.hybris.training.facades.customer.DefaultEcentaCustomerFacade;
import de.hybris.training.storefront.validation.EcentaProfileValidator;
...
public class AccountPageController extends AbstractSearchPageController
{
	...
	@Resource (name = "defaultEcentaCustomerFacade")
	private DefaultEcentaCustomerFacade defaultEcentaCustomerFacade;


	@Resource (name = "ecentaProfileValidator")
	private EcentaProfileValidator ecentaProfileValidator;
	...
	protected EcentaProfileValidator getEcentaProfileValidator()
	{
		return ecentaProfileValidator;
	}
	...
	public String editProfile(final Model model) throws CMSItemNotFoundException
	{
		...
		final CustomerData customerData = defaultEcentaCustomerFacade.getCurrentCustomer();
		...
		updateProfileForm.setPhoneNumber(customerData.getPhoneNumber());

		model.addAttribute("updateProfileForm", updateProfileForm);
		model.addAttribute("lastLogin", customerData.getLastLogin());
		model.addAttribute("ID", customerData.getDisplayUid());
		model.addAttribute("phone", customerData.getPhoneNumber());
			...
			final RedirectAttributes redirectAttributes) throws CMSItemNotFoundException
	{
		...
		getEcentaProfileValidator().validate(updateProfileForm, bindingResult);
		...
		final CustomerData currentCustomerData = defaultEcentaCustomerFacade.getCurrentCustomer();
		...
		customerData.setPhoneNumber(updateProfileForm.getPhoneNumber());
		...
		else
		{
			try
			{
				...
				defaultEcentaCustomerFacade.updateProfile(customerData);
				GlobalMessages.addFlashMessage(redirectAttributes, GlobalMessages.CONF_MESSAGES_HOLDER,
						"text.account.profile.confirmationUpdated", null);
